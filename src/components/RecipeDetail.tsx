...
export function RecipeDetail() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      getRecipe(params.id),
      getFavorites(),
    ]).then(([nextProfile, nextRecipe, nextFavorites]) => {
    ...
    <>
      ...
      <section className="mx-auto grid w-[min(1120px,calc(100%-32px))] grid-cols-1 gap-5 px-4 py-7 md:grid-cols-[0.85fr_1.15fr] md:px-0">
        <aside className="rounded-3xl border border-black/10 bg-white p-5 shadow-[0_18px_50px_rgba(31,37,32,0.08)]">
          ...
          <section className="mb-6 rounded-2xl border border-black/10 bg-[#fbf7ef] p-4">
            ...
            <div className="flex items-center gap-3">
              {recipe.authorAvatarUrl ? (
                ...
                <span className="grid h-12 w-12 place-items-center rounded-full bg-[#1f2520] text-sm font-black text-white">
                  {recipe.authorAvatar}
                </span>
              )}
              <div>
                <h2 className="text-base font-black text-[#1f2520]">
            ...
            {recipe.authorEmail ? (
              <p className="mt-3 wrap-break-word text-sm text-[#596159]">
                {recipe.authorEmail}
              </p>
            ) : null}
          </section>
          ...
          <div className="mb-5 grid gap-2 font-black text-[#596159]">
            ...
            <div className="inline-flex w-fit items-center gap-3 rounded-2xl border border-black/10 bg-white p-1 text-[#1f2520]">
              ...
              <span className="min-w-16 text-center text-lg font-black">
                {servings}
              </span>
              <button
            ...
            {recipe.ingredients.map((ingredient) => (
              <li key={`${ingredient.name}-${ingredient.unit}`}>
                <label className="flex items-baseline gap-3 leading-6">
                  <input type="checkbox" />
                  <span>
                    {formatQty(ingredient.qty * ratio)} {ingredient.unit}{" "}
                    {ingredient.name}
                  </span>
                </label>
              </li>
        ...
        <article className="rounded-3xl border border-black/10 bg-white p-5 shadow-[0_18px_50px_rgba(31,37,32,0.08)]">
          <p className="detail-print-title leading-8 text-[#596159]">
            {recipe.description}
          </p>
          ...
          <ol className="grid list-none gap-3 p-0">
            {recipe.steps.map((step, index) => (
              <li
                className="grid grid-cols-[34px_1fr] gap-3 leading-7"
                key={step}
              >
